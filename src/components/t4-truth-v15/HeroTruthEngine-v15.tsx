...
export function HeroTruthEngineV15() {
  ...
  // Use hovered step if hovering, otherwise keep the last active step
  const displayedStep = hoveredStep !== null ? hoveredStep : activeStep

  // Grid glow effect - NO random sparking dots
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Base opacity for always-visible grey grid (like Devin.ai)
      const baseGridOpacity = 0.035
      const normalOpacityBoost = 0.075
      const fastMovementBoost = speed > 15 ? Math.min(0.1, (speed - 15) / 40) : 0

      colorPhaseRef.current += 0.025

      // Draw grid lines - always visible with subtle grey, brighter on hover
      for (let x = 0; x <= canvas.width; x += gridSize) {
        ...
        if (intensity > 0 && speedMultiplier > 0.05) {
          ...
          ctx.strokeStyle = `rgba(120, 130, 150, ${baseGridOpacity})`
        }
  ...
  // Handler for hovering - sets the active step when hovering
  const handleHover = (index: number) => {
    setHoveredStep(index)
    setActiveStep(index) // Also update activeStep so it persists after leaving
  }
  ...
  return (
    <section className="relative min-h-screen bg-[#10131C] overflow-hidden">
      ...
      {/* Floating Navigation - reduced width on left, more transparent */}
      <header className="fixed top-12 left-0 right-0 z-50 px-6">
        <div className="mx-auto flex items-center" style={{ maxWidth: 'calc(100% - 48px)' }}>
          {/* Logo with more space before toolbar */}
          ...
          {/* Toolbar - more transparent for section 2 hover effect */}
          <nav className="flex-1 flex items-center justify-between rounded-xl border border-white/[0.08] bg-[#1A1D26]/50 backdrop-blur-xl px-2 py-2 transition-colors duration-300 hover:bg-[#3A4050]/40">
      ...
      {/* Hero Content - increased left margin and more top spacing */}
      <div className="relative z-10 pt-40 pb-20" style={{ padding: '180px 70px 80px 90px' }}>
        <div className="mx-auto" style={{ maxWidth: 'calc(100% - 140px)' }}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            ...
            <div className="max-w-2xl">
              ...
              >
                ...
                <span
                  className="bg-clip-text text-transparent"
                  style={{
                    backgroundImage: 'linear-gradient(90deg, rgb(110, 242, 204), rgb(75, 182, 211))',
                  }}
                >
                  ,
                </span>
                <span className="text-[#C8D1E0]"> Norges ledende</span>
                <br />
                <span className="text-[#C8D1E0]">B2B-dataplattform</span>
              </h1>
              ...
              <p className="mt-8 text-[#BAD7F5]/70 text-lg leading-relaxed">
                <span className="text-[#F2F5FA] font-medium">Finn, forstå og vinn</span>{' '}
                norske B2B-kunder med det mest komplette og sammenkoblede
                <br />
                universet av bedrifts-, person- og anbudsdata i Norge.
              </p>

              {/* Workflow Steps with arrow extend effect on hover */}
            ...
            {/* Right: Preview Panel with marble2.jpg background - FROZEN SIZE */}
            <div className="relative flex justify-center lg:justify-end" style={{ marginTop: '70px' }}>
              {/* Fixed size wrapper to prevent marble from shifting */}
              <div className="relative w-full max-w-[600px] h-[450px]">
                {/* Marble background image - marble2.jpg with FROZEN size using fixed dimensions */}
                <div
                ...
                {/* Subtle glow effect behind preview */}
                <div
                  ...
                  style={{
                    ...
                    right: '-30px',
                    bottom: '-30px',
                    background: 'radial-gradient(ellipse at center, rgba(75, 182, 211, 0.15) 0%, transparent 70%)',
                    filter: 'blur(20px)',
                  }}
                />

                {/* Preview panel - fixed height with scrollable content */}
                <div
      ...
      {/* Bottom fade glow effect - wide across the entire section, light blue */}
      <div
        className="absolute bottom-0 left-0 right-0 pointer-events-none"
        style={{
          width: '100%',
          height: '350px',
          background: 'radial-gradient(ellipse 120% 100% at 50% 100%, rgba(75, 182, 211, 0.35) 0%, rgba(100, 180, 220, 0.2) 25%, rgba(68, 109, 205, 0.15) 45%, transparent 75%)',
          filter: 'blur(50px)',
          zIndex: 5,
        }}
      />
...
function WorkflowStep({
  ...
  // Arrow extends from left to right when hovered - bigger extension
  const arrowExtend = isHovered ? 32 : (showActiveState ? 20 : 12)

  // Text shifts right when hovered
  ...
  return (
    <li
      ...
      style={{
        // Arrow shape with dynamic extension on hover - expands from left to right
    ...
    >
      ...
      {/* Complete outline - single element with rounded left corners */}
      <div
        ...
        style={{
          borderLeft: '1px solid rgba(75, 182, 211, 0.6)',
          borderTop: '1px solid rgba(75, 182, 211, 0.6)',
          borderBottom: '1px solid rgba(75, 182, 211, 0.6)',
          borderRadius: '12px 0 0 12px',
      ...
      {/* Content with slide animation on hover - text moves right */}
      <div
      ...
      <div
        ...
        style={{ transform: `translateX(${textShift}px)` }}
      >
        <p className={`text-[17px] font-medium transition-colors ${showActiveState ? 'text-[#F2F5FA]' : 'text-[#D0E0F5] group-hover:text-[#E8F0FA]'}`}>
          {title}
        </p>
