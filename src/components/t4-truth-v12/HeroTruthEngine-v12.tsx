...
export function HeroTruthEngineV12() {
  ...
  const sparkDotsRef = useRef<{ x: number; y: number; brightness: number; decay: number }[]>([])
  const [activeStep, setActiveStep] = useState(0)
  const [hoveredStep, setHoveredStep] = useState<number | null>(null)

  const displayedStep = hoveredStep !== null ? hoveredStep : activeStep

  // Grid glow effect with spark dots near mouse
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Mouse-tracking glow points with traveling brightness wave
      ...
      // Spark dots - brief bright flashes near mouse pointer
      // Update and draw existing spark dots
      sparkDotsRef.current = sparkDotsRef.current.filter(dot => {
        dot.brightness -= dot.decay
        if (dot.brightness <= 0) return false

        const gradient = ctx.createRadialGradient(dot.x, dot.y, 0, dot.x, dot.y, 15)
        gradient.addColorStop(0, `rgba(200, 255, 240, ${dot.brightness})`)
        gradient.addColorStop(0.3, `rgba(110, 242, 204, ${dot.brightness * 0.7})`)
        gradient.addColorStop(1, 'rgba(110, 242, 204, 0)')
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (deltaTime > 0) {
        ...
        // Create spark dots near the mouse when moving - random chance
        if (speed > 2 && Math.random() < 0.15) {
          // Find nearby grid intersections
          const nearbyGridX = Math.round(e.clientX / gridSize) * gridSize
          const nearbyGridY = Math.round(e.clientY / gridSize) * gridSize

          // Add some randomness to which nearby intersection sparks
          const offsetX = (Math.random() - 0.5) * 2 * gridSize
          const offsetY = (Math.random() - 0.5) * 2 * gridSize

          const sparkX = nearbyGridX + Math.round(offsetX / gridSize) * gridSize
          const sparkY = nearbyGridY + Math.round(offsetY / gridSize) * gridSize

          // Check if it's close enough to mouse (within ~2 grid cells)
          const distToMouse = Math.sqrt(Math.pow(sparkX - e.clientX, 2) + Math.pow(sparkY - e.clientY, 2))
          if (distToMouse < gridSize * 2.5 && sparkDotsRef.current.length < 5) {
            sparkDotsRef.current.push({
              x: sparkX,
              y: sparkY,
              brightness: 0.6 + Math.random() * 0.4,
              decay: 0.03 + Math.random() * 0.02, // Fast decay for brief flash
            })
          }
        }
      }
        ...
        <div className="mx-auto" style={{ maxWidth: 'calc(100% - 140px)' }}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="max-w-2xl">
              ...
              {/* Workflow Steps with v8 arrow shape and bright outline */}
            ...
            {/* Right: Preview Panel with marble1.jpg background */}
            <div className="relative flex justify-center lg:justify-end">
              <div className="relative w-full max-w-[600px]">
                {/* Marble background image - constant size for all previews */}
                <div
...
function WorkflowStep({
  ...
  // Only show active state for THIS specific step when it's hovered or active
  // This fixes the glitch where hovering step 2/3/4 would light up step 1
  const showActiveState = isHovered || (active && !isHovered)
  ...
  return (
    <li
      ...
      style={{
        // V8 arrow shape pointing right using clip-path
        clipPath: showActiveState
    ...
    >
      ...
      {/* Bright border glow - kept from v10 */}
      {showActiveState && (
        <div
          className="absolute inset-0 pointer-events-none"
          style={{
      ...
      <div
        ...
        } ${!showActiveState ? 'group-hover:translate-x-1' : ''}`}
      >
        {num}
      </div>
      <div className={`relative transition-all duration-300 ${!showActiveState ? 'group-hover:translate-x-1' : ''}`}>
