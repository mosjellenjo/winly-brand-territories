...
export function HeroTruthEngineV11() {
  ...
  // Grid glow effect with random bright dots
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Random bright dots - these light up randomly when mouse is hovering over section
      const now = Date.now()
      for (const dot of randomDotsRef.current) {
        // Update target brightness randomly
        if (now - dot.lastChange > 1500 + Math.random() * 2000) {
          dot.targetBrightness = speedMultiplier > 0.02 ? (Math.random() > 0.85 ? 0.4 + Math.random() * 0.4 : 0) : 0
          dot.lastChange = now
        }
        ...
        if (dot.brightness > 0.05) {
          ...
          gradient.addColorStop(0, `rgba(110, 242, 204, ${dot.brightness})`)
          gradient.addColorStop(0.4, `rgba(75, 182, 211, ${dot.brightness * 0.5})`)
          gradient.addColorStop(1, 'rgba(110, 242, 204, 0)')
        ...
        <div className="mx-auto" style={{ maxWidth: 'calc(100% - 140px)' }}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="max-w-2xl">
              ...
              {/* Workflow Steps with v8 arrow shape - FIXED hover glitch */}
            ...
            <div className="relative flex justify-center lg:justify-end">
              <div className="relative w-full max-w-[600px]">
                {/* Marble background image - rounded, slightly larger than preview */}
                <div
...
function WorkflowStep({
  ...
  // Use isHovered prop directly to determine visual state - fixes the glitch
  // where hovering step 2/3/4 would briefly light up step 1
