...
export function HeroTruthEngineV7() {
    ...
    const draw = () => {
      ...
      for (let x = 0; x <= canvas.width; x += gridSize) {
        ...
        ctx.strokeStyle = intensity > 0
          ? `rgba(75, 182, 211, ${0.03 + intensity * 0.15})`
          : 'rgba(68, 109, 205, 0.02)'
  ...
  return (
    <section className="relative min-h-screen bg-[#10131C] overflow-hidden">
      ...
      {/* Floating Navigation - rounded rectangle like Devin */}
      <header className="fixed top-12 left-0 right-0 z-50 px-10">
        <div className="mx-auto flex items-center" style={{ maxWidth: 'calc(100% - 80px)' }}>
          ...
          {/* Nav bar - rounded rectangle (not pill), more transparent */}
          <nav className="flex-1 flex items-center justify-between rounded-xl border border-white/[0.06] bg-[#10131C]/40 backdrop-blur-md px-2 py-2">
      ...
      <div className="relative z-10 pt-40 pb-20" style={{ padding: '160px 70px 80px 70px' }}>
        <div className="mx-auto" style={{ maxWidth: 'calc(100% - 140px)' }}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            {/* Right: Preview Panel with static marble background like Devin */}
            <div className="relative flex justify-center lg:justify-end">
              <div className="relative w-full max-w-[600px]">
                {/* Static marble/aurora background - like Devin's bright flowing effect */}
                <div
                  ...
                  style={{
                    background: `
                      linear-gradient(135deg,
                        ...
                        rgba(150, 200, 240, 0.3) 25%,
                        rgba(180, 230, 220, 0.35) 50%,
                        rgba(200, 220, 250, 0.3) 75%,
                        rgba(170, 200, 230, 0.4) 100%
                      )
                    `,
                    filter: 'blur(60px)',
                  }}
                ...
                {/* Secondary marble layer for depth */}
                <div
                  className="absolute -inset-4 rounded-2xl"
                  style={{
                    background: `
                      radial-gradient(ellipse at 20% 30%, rgba(200, 230, 245, 0.5) 0%, transparent 50%),
                      radial-gradient(ellipse at 80% 70%, rgba(180, 220, 240, 0.4) 0%, transparent 50%),
                      radial-gradient(ellipse at 50% 50%, rgba(190, 225, 235, 0.3) 0%, transparent 60%)
                    `,
                    filter: 'blur(30px)',
                  }}
                />
...
function NavItem({ label, active = false, hasArrow = false }: { label: string; active?: boolean; hasArrow?: boolean }) {
  return (
    <a
      href="#"
      className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-base transition-all ${
        active
          ? 'text-[#F2F5FA]'
