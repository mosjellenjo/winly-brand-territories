...
export function HeroTruthEngineV13() {
  ...
  const randomDotsRef = useRef<{ x: number; y: number; brightness: number; targetBrightness: number; lastChange: number }[]>([])
  const [activeStep, setActiveStep] = useState(0)
  const [hoveredStep, setHoveredStep] = useState<number | null>(null)

  const displayedStep = hoveredStep !== null ? hoveredStep : activeStep

  // Grid glow effect with random bright dots - INCREASED grid spacing, TONED DOWN dots
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      // Reinitialize random dots on resize
      initRandomDots()
    }

    // INCREASED grid spacing from 60 to 80
    const gridSize = 80
    const glowRadius = 300

    // Initialize random dots at grid intersections
    const initRandomDots = () => {
      const dots: typeof randomDotsRef.current = []
      for (let x = 0; x <= canvas.width; x += gridSize) {
        for (let y = 0; y <= canvas.height; y += gridSize) {
          dots.push({
            ...
            brightness: 0,
            targetBrightness: 0,
            lastChange: Date.now() + Math.random() * 2000, // Stagger initial changes
          })
        }
      }
      randomDotsRef.current = dots
    }
    ...
    const draw = () => {
      ...
      const normalOpacityBoost = 0.075
      const fastMovementBoost = speed > 15 ? Math.min(0.1, (speed - 15) / 40) : 0

      // Traveling color phase
      colorPhaseRef.current += 0.025

      // Draw grid lines with INCREASED spacing
      for (let x = 0; x <= canvas.width; x += gridSize) {
      ...
      // Mouse-tracking glow points with traveling brightness wave - TONED DOWN
        ...
        for (let x = 0; x <= canvas.width; x += gridSize) {
          for (let y = 0; y <= canvas.height; y += gridSize) {
            ...
            if (intensity > 0.1) {
              ...
              const gradient = ctx.createRadialGradient(x, y, 0, x, y, 6) // Smaller (was 8)
              // TONED DOWN alpha (was 0.25, now 0.15)
              const alpha = intensity * (speedMultiplier * 0.15 + fastMovementBoost * 0.3) * brightnessWave
      ...
      // Random bright dots - TONED DOWN brightness and frequency
      const now = Date.now()
      for (const dot of randomDotsRef.current) {
        // Update target brightness randomly - less frequent and dimmer
        if (now - dot.lastChange > 2000 + Math.random() * 3000) { // Longer intervals
          // Lower chance (was 0.85, now 0.92) and lower max brightness (was 0.4-0.8, now 0.15-0.35)
          dot.targetBrightness = speedMultiplier > 0.02 ? (Math.random() > 0.92 ? 0.15 + Math.random() * 0.2 : 0) : 0
          dot.lastChange = now
        }

        // Smooth transition to target
        dot.brightness += (dot.targetBrightness - dot.brightness) * 0.05

        if (dot.brightness > 0.03) {
          const gradient = ctx.createRadialGradient(dot.x, dot.y, 0, dot.x, dot.y, 8) // Smaller (was 12)
          gradient.addColorStop(0, `rgba(110, 242, 204, ${dot.brightness * 0.6})`) // Dimmer
          gradient.addColorStop(0.4, `rgba(75, 182, 211, ${dot.brightness * 0.3})`)
          gradient.addColorStop(1, 'rgba(110, 242, 204, 0)')
  ...
  return (
    <section className="relative min-h-screen bg-[#10131C] overflow-hidden">
      ...
      {/* Floating Navigation - SHRUNK toolbar with 3x space from logo */}
      <header className="fixed top-12 left-0 right-0 z-50 px-6">
        <div className="mx-auto flex items-center" style={{ maxWidth: 'calc(100% - 48px)' }}>
          {/* Logo with 3x MORE space before toolbar (was mr-3, now mr-9) */}
          ...
          {/* Nav bar - slightly smaller padding for shrunk toolbar */}
          <nav className="flex-1 flex items-center justify-between rounded-xl border border-white/[0.08] bg-[#1A1D26]/70 backdrop-blur-xl px-1.5 py-1.5">
            ...
            <div className="flex items-center gap-0.5">
              ...
              <a
                href="#"
                className="ml-1.5 inline-flex items-center justify-center px-5 py-2.5 text-sm font-normal text-[#10131C] rounded-lg transition-all hover:opacity-90"
      ...
      {/* Hero Content - LARGER left margin */}
      <div className="relative z-10 pt-40 pb-20" style={{ padding: '160px 70px 80px 100px' }}>
        <div className="mx-auto" style={{ maxWidth: 'calc(100% - 140px)' }}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="max-w-2xl">
              ...
              {/* Workflow Steps with CLEAN outline */}
            ...
            {/* Right: Preview Panel with marble1.jpg background - LARGER and HIGHER */}
            <div className="relative flex justify-center lg:justify-end">
              <div className="relative w-full max-w-[600px]">
                {/* Marble background image - LARGER and HIGHER */}
                <div
...
function NavItem({ label, active = false, hasArrow = false }: { label: string; active?: boolean; hasArrow?: boolean }) {
  return (
    ...
    >
      ...
      {label}
      {hasArrow && (
        <svg className="w-2.5 h-2.5 ml-px" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth={2}>
...
function WorkflowStep({
  ...
  // Use isHovered prop directly to determine visual state
  const isVisuallyActive = active || isHovered
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-0 transition-opacity duration-300 ${isVisuallyActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-50'}`}
        style={{
          background: 'linear-gradient(90deg, rgba(75, 182, 211, 0.1) 0%, rgba(75, 182, 211, 0.05) 50%, transparent 100%)',
        }}
      />

      {/* CLEAN border outline using pseudo-element approach */}
      {isVisuallyActive && (
        <>
          {/* Top border */}
          <div
            ...
            style={{
              width: 'calc(100% - 20px)',
              background: 'linear-gradient(90deg, rgba(75, 182, 211, 0.5) 0%, rgba(75, 182, 211, 0.3) 70%, rgba(75, 182, 211, 0.1) 100%)',
            }}
          />
          {/* Bottom border */}
          <div
      ...
      <div
        ...
        } ${!isVisuallyActive ? 'group-hover:translate-x-1' : ''}`}
      >
        {num}
      </div>
      <div className={`relative transition-transform duration-300 ${!isVisuallyActive ? 'group-hover:translate-x-1' : ''}`}>
        <p className={`text-base font-medium ${isVisuallyActive ? 'text-[#F2F5FA]' : 'text-[#BAD7F5]/60'}`}>
          {title}
        </p>
