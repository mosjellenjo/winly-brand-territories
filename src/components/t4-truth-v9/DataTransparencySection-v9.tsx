...
export function DataTransparencySectionV9() {
  const trustPoints = [
    {
      ...
      <div style={{ padding: '0 70px' }}>
        <div className="mx-auto" style={{ maxWidth: 'calc(100% - 140px)' }}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              <h2
                className="text-3xl sm:text-4xl text-[#F2F5FA] tracking-tight mb-6"
              ...
              <p
                className="text-lg text-[#BAD7F5]/60 leading-relaxed mb-10"
                style={{ fontFamily: 'Inter, system-ui, sans-serif', fontWeight: 300 }}
              >
              ...
              <div className="space-y-6">
                {trustPoints.map((point, index) => (
                  <div key={index} className="flex items-start gap-4">
                    ...
                    <div>
                      <h3
                        className="text-base text-[#F2F5FA] mb-1"
              ...
              <div className="relative rounded-xl border border-white/10 bg-[#0F1219] overflow-hidden p-8">
                <div className="space-y-4">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-[#BAD7F5]/50" style={{ fontFamily: 'Inter, system-ui, sans-serif', fontWeight: 300 }}>Datakilde</span>
                    <span className="text-[#4BB6D3]" style={{ fontFamily: 'Inter, system-ui, sans-serif', fontWeight: 400 }}>Brønnøysundregistrene</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-[#6EF2CC]" style={{ fontFamily: 'Inter, system-ui, sans-serif', fontWeight: 400 }}>Verifisert kilde</span>
                  </div>
                </div>
              </div>
            </div>
...
function DataRow({ label, value, verified = false }: { label: string; value: string; verified?: boolean }) {
  return (
    <div className="flex items-center justify-between">
      <span className="text-sm text-[#BAD7F5]/50" style={{ fontFamily: 'Inter, system-ui, sans-serif', fontWeight: 300 }}>{label}</span>
      <div className="flex items-center gap-2">
        <span className="text-sm text-[#F2F5FA]" style={{ fontFamily: 'Inter, system-ui, sans-serif', fontWeight: 400 }}>{value}</span>
        {verified && (
